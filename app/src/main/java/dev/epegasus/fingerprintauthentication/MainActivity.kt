package dev.epegasus.fingerprintauthentication
    ...
    private fun authenticateUser(biometricWeak: Int) {
        ...
        when (biometricManager.canAuthenticate(biometricWeak)) {
            ...
            BiometricManager.BIOMETRIC_ERROR_NO_HARDWARE ->
                showToast("There is no suitable hardware")
            BiometricManager.BIOMETRIC_ERROR_HW_UNAVAILABLE ->
                showToast("The hardware is unavailable. Try again later")
            BiometricManager.BIOMETRIC_ERROR_NONE_ENROLLED ->
                showToast("No biometric or device credential is enrolled")
            BiometricManager.BIOMETRIC_ERROR_SECURITY_UPDATE_REQUIRED ->
                showToast("A security vulnerability has been discovered with one or more hardware sensors")
            BiometricManager.BIOMETRIC_ERROR_UNSUPPORTED ->
                showToast("The specified options are incompatible with the current Android version")
            BiometricManager.BIOMETRIC_STATUS_UNKNOWN ->
                showToast("Unable to determine whether the user can authenticate")
        }
    }
